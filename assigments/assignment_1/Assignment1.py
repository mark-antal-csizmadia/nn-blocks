...
def plot_losses(history, model_type, reg_rate_l2, lr_initial):
    ...
    filename = f"assets/basic/loss_lambda_{reg_rate_l2}_lr_{lr_initial}_{model_type}.png"
...
def plot_accuracies(history, model_type, reg_rate_l2, lr_initial):
    ...
    filename = f"assets/basic/acc_lambda_{reg_rate_l2}_lr_{lr_initial}_{model_type}.png"
...
def viz_kernel(w, im_shape, model_type, reg_rate_l2, lr_initial, figsize=(8, 8)):
    ...
    plt.gca().axis('off')
    #plt.show()
    filename = f"assets/basic/w_viz_lambda_{reg_rate_l2}_lr_{lr_initial}_{model_type}.png"
    plt.savefig(filename)
    plt.close()
...
def run_with_hyperparams(x_train, y_train, x_val, y_val, x_test, y_test, reg_rate_l2, n_epochs, batch_size, lr_initial):
...
def test_grad_check(x_train, y_train):
    # Grad check
...
def main():
    ...
    # grad check first
    test_grad_check(x_train, y_train)

    hyperparams_dict = [
        {"reg_rate_l2": 0.0, "n_epochs": 40, "batch_size": 100, "lr_initial": 0.1},
        {"reg_rate_l2": 0.0, "n_epochs": 40, "batch_size": 100, "lr_initial": 0.001},
        {"reg_rate_l2": 0.1, "n_epochs": 40, "batch_size": 100, "lr_initial": 0.001},
        {"reg_rate_l2": 1.0, "n_epochs": 40, "batch_size": 100, "lr_initial": 0.001},
    ]
    ...
    for hyperparam_dict in hyperparams_dict:
        ...
        results_dict = run_with_hyperparams(x_train, y_train, x_val, y_val, x_test, y_test, reg_rate_l2, n_epochs, batch_size, lr_initial)

        results_list_of_dicts.append(results_dict)

    df = pd.DataFrame(results_list_of_dicts)
    path = "assets/basic/results.csv"
