...
def read_and_pre_process_data():
    print("*"*10)
    print("read_and_pre_process_data running...\n")
    path_to_file = "data/goblet_book.txt"
    ...
    y_train = y_encoding
    print("read_and_pre_process_data completed\n")

    return x_train, y_train, onehot_encoder, hpdata


def num_gradient_checking(x_train, y_train, onehot_encoder, hpdata):
    print("*" * 10)
    print("num_gradient_checking running...\n")
    batch_size = 25
    x_train, y_train, = x_train[:batch_size], y_train[1:batch_size + 1]
    ...
    print("num_gradient_checking completed\n")


def train_model(x_train, y_train, onehot_encoder, hpdata):
    print("*" * 10)
    print("train_model running...\n")
    init_params = {"coeff": 1.0, "mean": 0.0, "std": 0.01}
    kernel_h_initializer = NormalInitializer(seed=None, **init_params)
    bias_h_initializer = NormalInitializer(seed=None, **init_params)
    kernel_o_initializer = NormalInitializer(seed=None, **init_params)
...
def main():
    x_train, y_train, onehot_encoder, hpdata = read_and_pre_process_data()
    num_gradient_checking(x_train, y_train, onehot_encoder, hpdata)
    train_model(x_train, y_train, onehot_encoder, hpdata)
